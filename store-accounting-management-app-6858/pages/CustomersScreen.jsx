    ...
    useEffect(() => {
        if (searchQuery) {
            const filtered = customers.filter(customer => 
                customer?.field_1ydd0pkoo6bdtko00fy?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                customer?.field_ok6ku0rdj62dx35y68o?.includes(searchQuery)
            );
    ...
    return (
        <View style={{ flex: 1, backgroundColor: theme.background }}>
            <View style={{ paddingTop: 50, paddingHorizontal: 20, paddingBottom: 15, backgroundColor: theme.primary }}>
                <Text style={{ fontSize: 24, fontWeight: 'bold', color: '#FFFFFF', marginBottom: 15 }}>{t('customers')}</Text>
                    ...
                    filteredCustomers.map((customer) => (
                        ...
                        >
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 12, color: theme.textSecondary, marginBottom: 4 }}>{t('totalSales')}</Text>
                                    <Text style={{ fontSize: 16, fontWeight: 'bold', color: theme.text }}>
                                        {parseFloat(customer?.field_dr7jjokkyzfkmugzkgj || 0).toFixed(2)} ر.س
                                    </Text>
                                </View>
                                <View style={{ flex: 1, alignItems: 'center' }}>
                                    ...
                                    <Text style={{ fontSize: 16, fontWeight: 'bold', color: theme.success }}>
                                        {parseFloat(customer?.field_c4peyhuf33pdostvw9g || 0).toFixed(2)} ر.س
                                    </Text>
                                </View>
                                <View style={{ flex: 1, alignItems: 'flex-end' }}>
                                    ...
                                    <Text style={{ fontSize: 16, fontWeight: 'bold', color: theme.error }}>
                                        {parseFloat(customer?.field_wg7k9sbxsxh5ky1hh80 || 0).toFixed(2)} ر.س
                                    </Text>
                                </View>
                            </View>
                        </View>
